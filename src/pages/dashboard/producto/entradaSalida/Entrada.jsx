...
const Entrada = () => {
    ...
    const newEntrada = async (e) => {
        ...
        const {data} = await axios.post(`${api}/producto/registerentrada/${id}`, form, {headers: getHeaders(userData)})
        if(data.status == 200) {
            alert('se cargo correctamente la entrada')
  ...
  return (
    <div className="contentEntrada">
      <section className="createEntrada">
        ...
        <FormInputs createNewAction={newEntrada} action='entrada' entradas={totalEntrada} salidas={totalSalida}></FormInputs>
      </section>
      <section className="tablaDatos">
        <TableForm cantidad={entradas} action='entrada' deleteRegister={deleteRegister}></TableForm>
      </section>
    </div>
  );
};
